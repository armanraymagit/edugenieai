...
export async function storeStudyMaterials() {
  ...
  const materials = [
    ...
    },
    {
      id: 'js-async-1',
      content:
        'async/await is syntactic sugar for Promises. An async function always returns a Promise, and await pauses execution until the Promise resolves.',
      metadata: {
        topic: 'JavaScript',
        subtopic: 'Async',
        difficulty: 'intermediate',
...
export async function searchWithQualityFilter(query: string) {
  const vectorDb = getVectorDbService();
 ...
 */
export async function smartStudyAssistant(userQuestion: string) {
  const vectorDb = getVectorDbService();

  // 1. Find relevant study materials
  const relevantMaterials = await vectorDb.similaritySearchWithThreshold(userQuestion, 3, 0.4);
  ...
  const context = relevantMaterials.map((m) => m.content).join('\n\n');
...
export async function importFlashcardsToVectorDb(flashcards: any[]) {
  const vectorDb = getVectorDbService();
...
export async function clearOldStudyMaterials() {
  const vectorDb = getVectorDbService();
